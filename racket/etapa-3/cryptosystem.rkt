...
(define (key n)
  ...
  (let ((e (cadr cvartet)) (f (caddr cvartet)))        
    (foldr (lambda (x acc) (cons (modulo x 27) acc)) null (list (a1 e f) (b1 e f) (c1 e f) (a2 e f) (b2 e f) (c2 e f) (a3 e f) (b3 e f) (c3 e f)))))
...
(define (message->codes message)
  (define strtok (string->list message))
  (foldr (lambda (x acc) (if (equal? (- (char->integer x) 96) -64)
                             (cons (- (char->integer x) 32) acc)
                             (cons (- (char->integer x) 96) acc))) null strtok))
...
(define (codes->message codes)
  (list->string (foldr (lambda (x acc) (if (equal? x 0)
                                           (cons (integer->char (+ x 32)) acc)
                                           (cons (integer->char (+ x 96)) acc))) null codes)))
...
(define (extend-key-helper key initial-key size Result)
  (let ((copy-list initial-key))
    (if (equal? size 0)
        Result
        (if (null? key)
            (extend-key-helper copy-list initial-key size Result)
            (extend-key-helper (cdr key) initial-key (- size 1) (cons (car key) Result))))))
(define (extend-key key size)
  (reverse (extend-key-helper key key size '())))
...
(define (enc-dec-codes message key sign)
  (map sign message key))
(define encrypt-codes
  (lambda (message key)
    (foldr (lambda (x acc) (cons (modulo x 27) acc)) null (enc-dec-codes message (extend-key key (length message)) +))))
...
(define encrypt-message
  (lambda (message key)
    (let ((message (message->codes message)))
      (codes->message (foldr (lambda (x acc) (cons (modulo x 27) acc)) null (enc-dec-codes message (extend-key key (length message)) +))))))
