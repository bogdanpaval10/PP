...
outNeighbors node graph = S.fromList (S.foldr (\y acc -> (snd y) : acc) [] (S.filter (\x -> (fst x) == node) (edges graph)))
...
inNeighbors node graph = S.fromList (S.foldr (\y acc -> (fst y) : acc) [] (S.filter (\x -> (snd x) == node) (edges graph)))
...
removeNode node graph = fromComponents ns es
    where
        ns = S.toList (S.filter (\x -> x /= node) (nodes graph))
        es = S.toList (S.filter (\x -> (fst x) /= node && (snd x) /= node) (edges graph))
...
splitNode old news graph = fromComponents ns es
    where
        ns = S.foldr (\x acc -> (if x == old
                                    then acc ++ news
                                    else acc ++ [x])) [] (nodes graph)
        es = S.foldr (\x acc -> (if (fst x) == old
                                    then foldr (\y acc -> acc ++ [(y, (snd x))]) acc news
                                    else if (snd x) == old
                                            then foldr (\y acc -> acc ++ [((fst x), y)]) acc news
                                            else x : acc)) [] (edges graph)
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = fromComponents ns es
    where
        updatedListNodes = S.toList (S.filter (\x -> (not)(prop x)) (nodes graph))
        ns = if updatedListNodes == S.toList (nodes graph)
                then updatedListNodes
                else updatedListNodes ++ [node]
        es = S.foldr (\x acc -> (if prop (fst x) && prop (snd x)
                                    then acc ++ [(node, node)]
                                    else if prop (fst x)
                                            then acc ++ [(node, (snd x))]
                                            else if prop (snd x)
