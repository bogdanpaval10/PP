...
(define (apply-matrix-transformations Ts ppt f1 f2 f3)
  (if (equal? (length ppt) 3)
      (if (null? Ts)
          ppt
          (cond
            ((equal? (car Ts) 1) (apply-matrix-transformations (cdr Ts) (multiply f1 ppt) f1 f2 f3))
            ((equal? (car Ts) 2) (apply-matrix-transformations (cdr Ts) (multiply f2 ppt) f1 f2 f3))
            ((equal? (car Ts) 3) (apply-matrix-transformations (cdr Ts) (multiply f3 ppt) f1 f2 f3))))
      (if (null? Ts)
          ppt
          (cond
            ((equal? (car Ts) 1) (apply-matrix-transformations (cdr Ts) (f1 (car ppt) (cadr ppt) (caddr ppt) (cadddr ppt)) f1 f2 f3))
            ((equal? (car Ts) 2) (apply-matrix-transformations (cdr Ts) (f2 (car ppt) (cadr ppt) (caddr ppt) (cadddr ppt)) f1 f2 f3))
            ((equal? (car Ts) 3) (apply-matrix-transformations (cdr Ts) (f3 (car ppt) (cadr ppt) (caddr ppt) (cadddr ppt)) f1 f2 f3))))))

(define get-nth-tuple
  (lambda (n)
    (lambda (tuple-start)
      (lambda (f1 f2 f3)
        (apply-matrix-transformations (get-transformations n) tuple-start f1 f2 f3)))))
...
(define get-nth-ppt-from-GH-quadruples
  (lambda (n)
    (define result (get-nth-quadruple n))
    (list (* (car result) (cadddr result)) (* 2 (* (cadr result) (caddr result))) (+ (* (cadr result) (cadr result)) (* (caddr result) (caddr result))))))
